...
export function BdgadTestDetail({ testRunKey, testNumber, onBack }: BdgadTestDetailProps) {
		...
		<div className="space-y-6 p-6">
			...
			<div className="flex items-center justify-between">
				<div className="flex items-center gap-4">
					...
					<div>
						<h1 className="text-2xl font-bold">Chi tiết lần khám {testNumber}</h1>
                        <div className="text-right">
						<p className="text-md text-muted-foreground">Ngày thực hiện: {formatDate(testDetail.date)}</p>
					</div>
					</div>
				</div>
			</div>
				...
				<CardContent>
					<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
						<div className="space-y-1">
							...
							<p className="text-sm font-semibold text-gray-900">{testDetail.patient.name}</p>
						</div>
						<div className="space-y-1">
							<p className="text-sm font-medium text-gray-700">CCCD</p>
							<p className="text-sm font-mono text-gray-900">{testDetail.patient.citizenId}</p>
						</div>
						{testDetail.patient.dateOfBirth && (
							<div className="space-y-1">
								<p className="text-sm font-medium text-gray-700">Ngày sinh</p>
								<p className="text-sm text-gray-900">{formatDate(testDetail.patient.dateOfBirth)}</p>
							</div>
						)}
						...
						{testDetail.patient.address && (
							<div className="space-y-1 md:col-span-2 lg:col-span-4">
								...
								<p className="text-sm text-gray-900">{testDetail.patient.address}</p>
							</div>
						)}
					</div>
				</CardContent>
			...
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center gap-2">
						...
						<span className="text-gray-800 font-bold">Thông tin xét nghiệm</span>
					</CardTitle>
				</CardHeader>
				<CardContent>
					{testDetail.labCodes && testDetail.labCodes.length > 0 ? (
						<div className="space-y-4">
							{testDetail.labCodes.map((code, index) => (
								<div key={index} className="p-5 bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
									...
									{code.labcode ? (
										<div className="space-y-4">
											<div className="grid gap-6 md:grid-cols-2">
												<div className="flex items-start gap-3">
													...
													<div>
														<h4 className="font-bold text-gray-800 mb-1">Lab Code</h4>
											...
											<div className="flex flex-wrap gap-3 pt-3 border-t border-gray-200">
												{code.file_url && (
													...
													>
														...
														{downloadingFiles.has(code.file_url) ? "Đang tải..." : "Tải File"}
													</Button>
												)}
												{code.excelResult && (
													<Button
										...
										<div className="space-y-4">
											<div className="flex items-start gap-3">
												...
												<div>
													<h4 className="font-bold text-gray-800 mb-1">Loại tài liệu</h4>
													<p className="text-sm font-semibold text-gray-800 bg-gray-100 px-3 py-1 rounded-full inline-block">
														{code.type === "general" ? "Tài liệu chung" : code.type}
													</p>
												</div>
											</div>
											...
											{code.file_urls && code.file_urls.length > 0 && (
												<div className="flex flex-wrap gap-3 pt-3 border-t border-gray-200">
													...
													return (
															...
															>
																<Download className="h-4 w-4" />
																{downloadingFiles.has(fileUrl) ? "Đang tải..." : `Tải ${fileName}`}
															</Button>
														)
													})}
												</div>
						...
						<div className="text-center py-12">
							...
							<p className="text-sm font-medium text-gray-500">Chưa có thông tin xét nghiệm</p>
						</div>
					)}
				</CardContent>
			</Card>
