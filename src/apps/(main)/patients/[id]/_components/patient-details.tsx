...
export function PatientDetails({ patient, onBack }: PatientDetailsProps) {
  ...
  const [selectedBdgadTest, setSelectedBdgadTest] = useState<{
    testRunKey: number;
    testNumber: number;
  } | null>(null);
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {patientDetails?.extendedInfo?.data?.patient
                        ?.date_of_birth
                        ? formatDate(
                            patientDetails.extendedInfo.data.patient
                              .date_of_birth
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium">CCCD</p>
                    <p className="text-sm text-muted-foreground font-mono">
                      {patientDetails?.extendedInfo?.data?.patient
                        ?.citizen_id || "Chưa cập nhật"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium">Quốc gia / Dân tộc</p>
                    <p className="text-sm text-muted-foreground">
                      {patientDetails?.extendedInfo?.data?.patient?.nation ||
                        "Chưa cập nhật"}{" "}
                      /{" "}
                      {patientDetails?.extendedInfo?.data?.patient?.ethnicity ||
                        "Chưa cập nhật"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-sm font-medium">Tình trạng hôn nhân</p>
                    <Badge variant="secondary" className="mt-1">
                      {patientDetails?.extendedInfo?.data?.patient
                        ?.marital_status || "Chưa cập nhật"}
                    </Badge>
                  </div>
                </div>
              </div>
